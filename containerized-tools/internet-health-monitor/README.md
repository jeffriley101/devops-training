...
A basic website checker can report whether a service is up or down.

This project goes further by treating monitoring as an operational platform workflow:
...
The application itself is organized around a simple monitoring pipeline:
...
This sample image represents the style of latency trend artifact generated by the platform from stored run history.
...
* packaging a Python workload into a repeatable Docker container
* keeping ECS task definitions, images, environment variables, and schedules aligned
* using EventBridge Scheduler for recurring serverless-style container execution
* using S3 as part of an artifact pipeline, not just file storage
* using CloudWatch logs to debug real scheduled runs in AWS
