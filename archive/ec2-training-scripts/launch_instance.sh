...
#Make the security group converge
converge_sg_ingress() {
  local sg_id="$1"
  local ssh_cidr="$2"  # e.g. "99.119.213.233/32"

  # Desired rules (tcp only)
  declare -A desired=()
  desired["tcp|22|22|${ssh_cidr}"]=1
  desired["tcp|80|80|0.0.0.0/0"]=1

  # Existing ingress rules (tcp + IPv4 only)
  # Output: rule_id  from  to  cidr
  mapfile -t existing_lines < <(
    aws ec2 describe-security-group-rules \
      --region "$REGION" \
      --filters "Name=group-id,Values=${sg_id}" \
      --query 'SecurityGroupRules[?IsEgress==`false` && IpProtocol==`tcp` && CidrIpv4!=`null`].[SecurityGroupRuleId,FromPort,ToPort,CidrIpv4]' \
      --output text \
      | sed '/^None$/d' || true
  )

  declare -A existing_keys_by_id=()
  declare -A existing_present=()

  local line rule_id from to cidr key
  for line in "${existing_lines[@]}"; do
    # AWS CLI text output is tab-separated
    IFS=$'\t' read -r rule_id from to cidr <<<"$line"
    key="tcp|${from}|${to}|${cidr}"
    existing_keys_by_id["$rule_id"]="$key"
    existing_present["$key"]=1
  done

  # 1) Revoke extras (present but not desired)
  local rid
  for rid in "${!existing_keys_by_id[@]}"; do
    key="${existing_keys_by_id[$rid]}"
    if [[ -z "${desired[$key]+x}" ]]; then
      if [[ "$PLAN" -eq 1 ]]; then
        plan "Would revoke SG ingress rule (drift): ${rid} (${key})"
      else
        log "Revoking drift SG ingress rule: ${rid} (${key})"
  ...
  # 2) Authorize missing desired rules
  local dkey proto dfrom dto dcidr
  for dkey in "${!desired[@]}"; do
    if [[ -z "${existing_present[$dkey]+x}" ]]; then
      IFS='|' read -r proto dfrom dto dcidr <<<"$dkey"
      if [[ "$PLAN" -eq 1 ]]; then
        plan "Would authorize SG ingress: ${proto} ${dfrom}-${dto} from ${dcidr}"
      else
        log "Authorizing SG ingress: ${proto} ${dfrom}-${dto} from ${dcidr}"
        aws ec2 authorize-security-group-ingress \
          --region "$REGION" \
          --group-id "$sg_id" \
          --ip-permissions "IpProtocol=${proto},FromPort=${dfrom},ToPort=${dto},IpRanges=[{CidrIp=${dcidr}}]" \
        >/dev/null
      fi
    fi
  done
...
MATCH_COUNT="$(aws ec2 describe-instances --region "$REGION" \
  --filters \
    "Name=tag:Name,Values=${TAG_PREFIX}-*" \
    "Name=instance-state-name,Values=running,pending,stopped,stopping" \
...
if [[ "${MATCH_COUNT}" != "None" && "${MATCH_COUNT}" -gt 1 ]]; then
  log "Warning: ${MATCH_COUNT} instances match ${TAG_PREFIX}-*. Using newest by LaunchTime."
fi
...
log "Converging SG ingress: SSH from ${SSH_CIDR}, HTTP from 0.0.0.0/0"
converge_sg_ingress "$SG_ID" "$SSH_CIDR"
log "DEBUG: REGION=${REGION}"
log "DEBUG: SG_ID=${SG_ID} SSH_CIDR=${SSH_CIDR}"
