...
def request(origin, host="woodshed.example", url_netloc="internal:8000", forwarded=None):
    ...
    return SimpleNamespace(
        ...
        url=SimpleNamespace(netloc=url_netloc, scheme="https"),
    )
...
def test_null_origin_requires_same_origin_fetch_metadata_and_host(monkeypatch):
    monkeypatch.setenv("PUBLIC_BASE_URL", "https://woodshed.example/")
    valid = request("null", host="woodshed.example")
    valid.headers = Headers({"host": "woodshed.example", "origin": "null", "sec-fetch-site": "same-origin"})
    check_csrf(valid, "token")

    for headers in (
        {"host": "woodshed.example", "origin": "null", "sec-fetch-site": "cross-site"},
        {"host": "woodshed.example", "origin": "null"},
        {"host": "evil.example", "origin": "null", "sec-fetch-site": "same-origin"},
    ):
        with pytest.raises(HTTPException, match="Invalid request origin"):
            check_csrf(SimpleNamespace(headers=Headers(headers), session={"membership_csrf": "token"},
                                       url=SimpleNamespace(netloc="internal:8000", scheme="https")), "token")
