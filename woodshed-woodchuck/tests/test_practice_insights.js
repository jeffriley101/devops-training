...
const response = {ok: true, status: 200, json: async () => ({weeks: [{
  ...
  verified_minutes: 10, pristine_minutes: 0}], total_minutes: 15, average_weekly_minutes: 3.75})};
...
test('Full panel renders isolated aggregate via no-store request', async () => {
  ...
  assert.match(ui.elements.content.children[1].textContent, /Weekly average: 3.75/);
